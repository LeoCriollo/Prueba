...
function GuardarFae(estado) {
    ...
    var tableData = miTablaFAE.rows().data();

    formData.append("codigoEmpleado", $("#codigoempleado").val());
    formData.append("secuencial",DetGastos.secuencial);
    formData.append("destino", $("#slDestino option:selected").val());
    formData.append("motivoAnticipo", $("#motanticefect option:selected").val());
    formData.append("pais", $("#SlPais option:selected").val());
    formData.append("AutoFinanciera", $("#SlctGerentes option:selected").val());
    formData.append("ciudad", $("#Ciudades option:selected").val());
    formData.append("usaTc", $("#slTC option:selected").val());
    formData.append("Comentario", $("#Comentarios").val());
    formData.append("fechadesde", $("#fechadesde").val());
    formData.append("fechahasta", $("#fechahasta").val());
    formData.append("nivel1", $("#txtareanivel2").val());
    formData.append("nivel2", $("#txtareanivel3").val());
    formData.append("cantInvitados", conteoAsist);
    formData.append("accion", estado);
    formData.append("estado", estado);
    formData.append("idFile", idFile);
    formData.append("archivoprueba", $("#fileAprobacion")[0].files[0]);
    var index = 0;
    $("input[id^='asistentes-']").each(function () {
        const nombre = $(this).val();

        if (nombre.length > 3 && nombre !== null) {
            formData.append("ArrayAsistente[" + index + "].nombre", nombre);
            index++;
        }
    });
    tableData.each(function (data, index) {
        ...
        formData.append("DetallGasto[" + index + "].idtipoconsumo", data[6])
        formData.append("DetallGasto[" + index + "].valor", data[1])
        formData.append("DetallGasto[" + index + "].total", data[5])
        formData.append("DetallGasto[" + index + "].descripcionGastos", data[4])
...
function Guardar(estado) {
    ...
    if ($("#nombre").val() != null && DetConsumo.DetallConsumo.length > 0) {
        ...
        formData.append("secuenExterno", $("#SlctLiquidacionFae option:selected").val());
...
function CodigoEmpleado() {
    ...
    request.done(function (tpm) {
        ...
        if (tpm.Bandera != "V") {
            ...
            console.log("llego")
            if ($("#slctTipo").length > 0) {
                // Si existe, deshabilita la opción seleccionada
        $("#slctTipo").prop("disabled", false);
...
function CalcularTotal() {
    ...
    if (fechah != "" && fechad != "") {
        ...
        calFecha = fecha2.diff(fecha1.add(-1, 'days'), 'days');
        $("#CalculoFecha").empty();
    ...
    if (fechah != "" && fechad != "" && valor != "") {
        ...
        total = calFecha * valor * conteoAsist;
    ...
    return { total: total, calFecha: calFecha };
}

function LimpiarVariableFAE() {
    document.ready = document.getElementById("tipoconsumo").value = '0';
    $("#valor").val("");
    $("#total").val("");
    $("#descripcionGastos").val("");
