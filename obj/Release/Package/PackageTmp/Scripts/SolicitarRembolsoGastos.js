...
function CalcularValores(tableData) {
    ...
    if (tableData.length > 0) {
        ...
        tableData.each(function (rowData, index) {
            ...
            if (rowData[5] > 15 && rowData[18] == null) {
            ...
            valorReportado = valorReportado + parseFloat(rowData[5]);
            //rowData[15] = idTipoconsumo
            if (rowData[15] == 5 || rowData[15] == 11) {


                if (rowData[15] == 5) {
                    ConsumosCombustibles = ConsumosCombustibles + parseFloat(rowData[5]);
                } else {
                    ServiciosFinancieros = ServiciosFinancieros + parseFloat(rowData[5]);
                }


            } else {
                if (rowData[20] == 1) {
                    Nacional = Nacional + parseFloat(rowData[5]);
                } else {
                    Exterior = Exterior + parseFloat(rowData[5]);
                }
    ...
    //        if (detalle[i].valor > 15 && detalle[i].namefile == "") {
    //            if (entro == 0) {
    //                entro = 1;
