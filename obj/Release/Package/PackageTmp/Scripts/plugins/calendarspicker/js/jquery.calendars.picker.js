...
(function ($) { // Hide scope, no $ conflict
    ...
    //JMurillot
    //creamos lista de variables para aplicar clase css segun la semana que corresponda
    var classWList = [];
    //Variable que contiene la clase a cargar.
    for (var i = 1; i <= 8; i++) {
        classWList.push({
            id: i,
            classWN: "calendars-week_" + i,
            ini: false,
            month: 1,
            week: 0,
            index: i-1
    ...
    classWList[0].ini = true; //inicializa
    //   
...
$(selector).calendarsPicker({minDate: 0, maxDate: '+1m +1w'}) */
    $.JQPlugin.createPlugin({
        ...
        defaultRenderer: {
            ...
            weekHeader: '<tr><th class="calendars-week">' + // JMurillot - se carga automaticamente la semana
			'<span title="{l10n:weekStatus}">{l10n:weekText}</span></th>{days}</tr>',
            dayHeader: '<th>{day}</th>',
            week: '<tr><td class="{classWeek}">{weekOfYear}</td>{days}</tr>',// JMurillot - se carga automaticamente la semana --Se agreaga {classWeek} para cargar css en la funcion _generateMonth
            day: '<td>{day}</td>',
...
$(selector).datepick('setDate', [date1, date2, date3]) */
  ...
  $('div.datepick-popup a:contains(10)')[0]) */
        ...
        /** JMurrilloT.
            Metodo para corregir la semana >=52 no se muestre en el mes de enero ni diciembre
			@memberof CalendarsPicker
			@private			
			@param {number} mes a evaluar.
			@param {number} semana a evaluar.			
			@return {bool} esSemanam determina si cumple la condicion. */
        _CalculateWeek_Dole: (function (mes, semana) {

            var esSemanam = false;
            if ((mes == 1 || mes == 12) && semana >= 52) {
                esSemanam = true;
            }
            return esSemanam;
        ...
        _generateMonth: function (elem, inst, year, month, calendar, renderer, first) {
            ...
            for (var week = 0; week < numWeeks; week++) {
                ...
                var weekOfYear = (!showWeeks ? '' : '<span class="jd' + jd + '">' +
                    ...
                    //JMurrilloT.
                    //Se modifica evaluacion para llamar al metodo que recalcula la semanas mayores a 52
                    this._CalculateWeek_Dole(month, drawDate.weekOfYear()) ? 1 : drawDate.weekOfYear() + 1
                    //drawDate.weekOfYear()
                    ) + '</span>');

                //JMurrilloT
                //Asignar clase para los colores de la semanas
                var classWeek = '';
                var weekY = drawDate.weekOfYear();
                //console.log()
                for (var i = 0; i < classWList.length; i++) {
                    if (classWList[i].ini == true) {
                        var index = classWList[i].index;
                        ...
                        if (classWList[i].month < month && classWList[i].week == weekY) {

                            if (i == 0) {
                                classWeek = classWList[7].classWN;

                            } else {
                                classWeek = classWList[index - 1].classWN;

                            }

                            // console.log(classWList[i]);

                        } else {

                            classWeek = classWList[i].classWN;
                            classWList[i].ini = false;

                            if (i == 7) {
                                classWList[0].ini = true;
                                classWList[0].month = month;
                                classWList[0].week = weekY;

                            }
                            else {
                                classWList[index + 1].ini = true;
                                classWList[index + 1].month = month;
                                classWList[index + 1].week = weekY;
                            }
                        }
                ...
                weeks += this._prepare(renderer.week, inst).replace(/\{days\}/g, days).
                   ...
                   .replace(/\{classWeek\}/g, classWeek); //JmurilloT se agrega logica para asignar clase css para los colores de DOLE en el calendario
            }
